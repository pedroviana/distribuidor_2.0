...
ActiveAdmin.register_page "Log Evento" do
      ...
      column do
        ...
        panel "Últimas atividades" do
          ...
          # Reversing the array to show the most recent updates
      		...
      		table_for PaperTrail::Version.where('item_type = "Event"').order('created_at DESC').each do |p|
      		  
      		  # Column to show the user that make the changes
      		  # =============================================
        		column "Quem" do |p|
        		...
        		column "O que" do |p|
        		  ...
        		  if record
        			  link_to record.title, admin_event_path(record) 
        		...
        		column "De" do |p|
        			...
        			p.changeset.each do |key, changes|
        			  ...
        			  if p.event == 'create'
        				...
        				field_name = I18n.t("activerecord.attributes.#{p.item_type.downcase}.#{key}")
        				append     << "<strong> #{field_name}:</strong> &nbsp;#{PaperTrailModel.correct_value(p.item_type.clone, key.clone, value.clone)}"
        			end
            ...
            # =================================
              ...
              link_to "#{p.reify.title}", undelete_admin_event_path(p) if p.event == 'destroy'
      		  end
            # =================================
        	end
        end     
